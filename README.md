...
| 文件              | 主要功能 |
|-------------------|--------------------------------------------------|
| CORE/loop_core.py  | 核心循环、Judge 调用、目标处理、状态管理、回调机制 |
| CORE/json_utils.py | JSON 解析、编码修复、流式处理                   |
| CORE/prompts.py    | Judge/Refine/Summarize 等系统提示词              |
| CORE/token_stats.py| Token 统计、LoopState/RoundLog 数据结构          |

#### TUI版本

| 文件              | 主要功能 |
|-------------------|--------------------------------------------------|
| TUI/loop.py       | TUI 主循环，命令行交互，调度核心逻辑             |
| TUI/display.py    | 终端输出美化、状态栏、格式化打印                 |

#### GUI版本

| 文件              | 主要功能 |
|-------------------|--------------------------------------------------|
| GUI/gui.py        | GUI主程序，基于tkinter的图形界面，双列显示       |

#### 主要流程与职责

- **CORE 核心模块**（CORE/loop_core.py）：实现自循环、目标管理、Judge 评估与决策，通过回调机制与UI层通信。
- **TUI 主循环**（TUI/loop.py）：负责命令行交互、用户输入、状态栏刷新，使用CORE模块的回调。
- **GUI 界面**（GUI/gui.py）：提供图形化操作界面，双列显示工具调用日志和文本输出，使用CORE模块的回调。
- **输出与美化**（TUI/display.py）：负责所有终端输出、颜色、宽度、状态栏等。
- **JSON 工具**（CORE/json_utils.py）：增强 JSON 解析能力，处理编码与流式数据。
- **提示词管理**（CORE/prompts.py）：集中管理 Judge/Refine/Summarize 等提示词。
- **Token 统计**（CORE/token_stats.py）：统计各类 Token 消耗，记录循环日志。
